...
class FormBuilder
{
    ...
    public function build($type, $name, $value = null,$attr = [])
    {
        if($type == 'view'){
            return '<div style="padding-top: 7px;">'. $value .'</div>';
        }
        if($type == 'qrcode'){
            $type = 'image';
        }
        ...
        if($type == 'fieldlist'){
            ...
            $title  = $attr ? json_decode($attr,true) : [];
            return $this->fieldlist($name, $value, $title);
        }
   
        return $this->$type($name,$value,$attr);
