...
use app\admin\model\yunos\Safetoken;
use fast\Random;
use fast\Tree;
use think\Config;
use think\Cookie;
...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        /*$safeToken = Safetoken::where(['user_id' => $admin->id ,'status' => 1])->select();
        if($safeToken){
            if(!Request()->post('kmcode')){
                $this->setError('请输入KM令');
                return false; 
            }
            
            $checkToken = false;
            foreach ($safeToken as $km){
                
                $kmcode = $this->kmToken($km);
                if(Request()->post('kmcode') == $kmcode){
                    $checkToken = true;
                    break;
                }
    ...
    public function kmToken($data)
    {
        ...
        $sha1 = sha1($str);
        return getFirstSixCombinedDigits($sha1);
    ...
    public function getYunosMenu($params = [], $fixedPage = 'dashboard')
    {
        ...
        foreach ($ruleList as $index => &$item) {
            ...
            $item['data_url'] = '../'.$item['name'];
        }
        
        $menu = $this->buildMenuTree($ruleList);
        
        return $menu;
        
    }
    
    public function buildMenuTree(array $items, $parentId = 0, $parentNodeId = '',$parentAppNode = '') {
        $menuTree = array();
     
        foreach ($items as $item) {
            if ($item['pid'] == $parentId) {
                // 构造当前节点的nodeId
                $nodeId = ($parentNodeId ? $parentNodeId . '-' : '') . $item['id'];
                
                $appNode = ($parentAppNode ? $parentAppNode . '-' : '') . sha1($item['id']);
     
                // 递归构建子节点
                $childItems = $this->buildMenuTree($items, $item['id'], $nodeId,$appNode);
