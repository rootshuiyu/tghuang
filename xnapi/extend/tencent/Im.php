...
class Im
{
    ...
    private $parm_url = [
        ...
        'DEL_GROUP' => 'v4/group_open_http_svc/destroy_group', //解散群组
        'LIST_GROUP' => 'v4/group_open_http_svc/get_appid_group_list', //获取 App 中的所有群组
        //群组管理
        ...
        //好友管理
        'FR_ADD' => 'v4/sns/friend_add',    //添加好友
        'FR_IMPORT' => 'v4/sns/friend_import', //导入好友
        'FR_DEL' => 'v4/sns/friend_delete',  //删除好友
        'FR_CHECK' => 'v4/sns/friend_check',  //校验好友
        'FR_GET' => 'v4/sns/friend_get',  //拉取好友
        
        'FR_BLACK_ADD' => 'v4/sns/black_list_add', //添加黑名单
        'FR_BLACK_DEL' => 'v4/sns/black_list_delete', //删除黑名单
        'FR_BLACK_GET' => 'v4/sns/black_list_get',  //拉取黑名单
        
        'SEND_GROUP_MSG' => 'v4/group_open_http_svc/send_group_msg', //推送群消息
        
        'IS_ONLINE' => 'v4/openim/query_online_status' //在线状态
    ...
    public function __construct($sdkappid='', $key='')
    {
        ...
        $this->sdkappid = Config::get('site.tx_im_appid');
        $this->key = Config::get('site.tx_im_key');
        
        $this->random = time();
        $this->admin_id = 'administrator';
    }
    ...
    public function onlinede(string $uid)
    {
        $url = $this->set_url('IS_ONLINE');
        $data = array(
            'To_Account' => [$uid],
        );
        $data = $this->curl_get($url, json_encode($data));
        $status = $data['QueryResult'][0]['Status'];
        if($status == 'Online'){
            return true;
        }
        return false;
    ...
    public function onlinearr(array $uid)
    {
        $url = $this->set_url('IS_ONLINE');
        foreach ($uid as &$v){
            $v = "$v";
    ...
    //删除账号 单次最多100人
    public function account_delete($item)
    {
        $url = $this->set_url('ACCOUNT_DELETE_URL');
        foreach ($item as $v){
            $res[] = ['UserID' => "{$v}"];
        }
        $data = ['DeleteItem' => $res];
    ...
    public function notice(string $to_id, $content)
    {
        $url = $this->set_url('SENDMSG_URL');
        $datas = array(
            'SyncOtherMachine' => 1,
     ...
     * @return bool ture成功 false失败
     */
    public function delete_work_user(string $work_name, string $uid)
    {
     ...
     * @return bool ture成功 false失败
     */
    public function msg_read(string $from_id, string $to_id)
    {
    ...
    //
    public function send_group_msg($to,$msg)
    {
        $msg = json_encode($msg);//JSON_PRETTY_PRINT
        $url = $this->set_url('SEND_GROUP_MSG');
    ...
    public function send_all_msg($msg)
    {
        $msg = json_encode($msg,JSON_PRETTY_PRINT);
        $url = $this->set_url('SEND_GROUP_MSG');
        
        $data = array(
            'GroupId' => '@TGS#a2I2XQJO6',
            'Random' => time(),
            'MsgBody' => [
                [
                    'MsgType' => 'TIMTextElem',
     ...
     */
    public function create_work(string $create_id, $group_name, $private = 'AVChatRoom', $ApplyJoinOption = 'FreeAccess')
    {
    ...
    //获取所有群组
    public function send_bro()
    {
        $url = $this->set_url('SEND_BRO');
        $data = [
            'From_Account' => '',
    ...
    //添加好友
    public function friend_add($userid,$to_id)
    {
        $url = $this->set_url('FR_ADD');
        $data = [
            'From_Account' => "{$userid}",//发起人
            'AddFriendItem' => [[
                'To_Account' => "{$to_id}",//被加的人
                'AddSource' => 'AddSource_Type_APP',//加好友来源字段
            ]],
        ];
        $response = $this->curl_get($url,json_encode($data));
    ...
    //导入好友
    public function friend_import($userid,$to_id)
    {return;
        $url = $this->set_url('FR_IMPORT');
        $data = [
            'From_Account' => "{$userid}",//发起人
            'AddFriendItem' => [[
                'To_Account' => "{$to_id}",//被加的人
    ...
    //校验好友
    public function friend_check($userid,$to_id)
    {
        $url = $this->set_url('FR_CHECK');
        $data = [
            'From_Account' => "{$userid}",//发起人
            'To_Account' => ["{$to_id}"],
        ...
        $Relation = $response['InfoItem'][0]['Relation'];
    ...
    //好友列表
    public function friend_get($userid)
    {
        $url = $this->set_url('FR_GET');
        $data = [
            'From_Account' => "{$userid}",//发起人
            //'StartIndex' => 2,
            //'StandardSequence' => 0
        ...
        $result = [];
        foreach ($response['UserDataItem'] as $v){
            $result[] = $v['To_Account'];
        }
        return $result;
    }
    ...
    //删除好友 - 双向
    public function friend_delete($userid,$to_id)
    {
        $url = $this->set_url('FR_DEL');
        $data = [
            'From_Account' => "{$userid}",//发起人
            'To_Account' => ["{$to_id}"],
    ...
    //添加黑名单
    public function friend_black_add($userid,$to_id)
    {
        $url = $this->set_url('FR_BLACK_ADD');
        $data = [
            'From_Account' => "{$userid}",//发起人
            'To_Account' => ["{$to_id}"]
        ];
    ...
    //添加黑名单
    public function friend_black_del($userid,$to_id)
    {
        $url = $this->set_url('FR_BLACK_DEL');
        $data = [
            'From_Account' => "{$userid}",//发起人
            'To_Account' => ["{$to_id}"]
        ];
    ...
    //拉取黑名单
    public function friend_black_get($userid)
    {
        $url = $this->set_url('FR_BLACK_GET');
        $data = [
            'From_Account' => "{$userid}",//发起人
            'StartIndex' => 0,
            'MaxLimited' => 1000
        ...
        $result = [];
        foreach ($response['BlackListItem'] as $v){
            $result[] = $v['To_Account'];
        }
        return $result;
    }
    ...
    public function genUserSig(string $userid, $expire = 86400 * 18000)
    {
    ...
    public function genPrivateMapKey(string $userid, $expire, $roomid, $privilegeMap)
    {
    ...
    public function genPrivateMapKeyWithStringRoomID(string $userid, $expire, $roomstr, $privilegeMap)
    {
