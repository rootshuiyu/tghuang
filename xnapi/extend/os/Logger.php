...
class Logger {
    private $logName;
    private $logId;
    /** @var string 日志目录，来自 config/backend.php 或 runtime/yunos_logs */
    private $logDir;

    public function __construct() {
        $dir = \config('backend.yunos_log_dir');
        $this->logDir = $dir !== '' && $dir !== null
            ? rtrim($dir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR
            : (function_exists('runtime_path') ? runtime_path('yunos_logs') . DIRECTORY_SEPARATOR : (__DIR__ . '/../../runtime/yunos_logs/'));
    }
    ...
    public function save($type,$title,$content = null) {
        ...
        // 写入文件
        file_put_contents($file, json_encode($existingData,JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT )); //JSON_PRETTY_PRINT
